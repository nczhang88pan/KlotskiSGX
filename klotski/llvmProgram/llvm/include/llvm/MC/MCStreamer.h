...
public:
  ...
  class CurFunctionStatic{
  private:
        MachineFunction* currentMF;
        MCFragment* funcFragmentStart;
        MCSection::iterator funcFragmentStartIter;
        MCPage* currentPage;
        unsigned prevFuncInstrEndSecOffset;
        unsigned instrStartSecOffset;
        unsigned size;
  public:
        CurFunctionStatic():currentMF(NULL),
            funcFragmentStart(NULL),currentPage(NULL){};
        ~CurFunctionStatic(){};

        void setCurrentMF(MachineFunction *MF){
            unsigned int align = MF->getAlignment();
            currentMF=MF;
            instrStartSecOffset = (prevFuncInstrEndSecOffset & ((1<<align)-1))?
                                             (prevFuncInstrEndSecOffset + (1<<align)) & ~((1<<align)-1) :
                                             prevFuncInstrEndSecOffset;
            //errs()<<" align "<<align;
            size=0;
        }
        void setStartFragmentIter(MCSection::iterator I){
            funcFragmentStartIter=I;
        }
        MCSection::iterator getStartFragmentIter(){
            return funcFragmentStartIter;
        }
        void setStartFragment(MCFragment* MCF){funcFragmentStart=MCF;}
        void setCurrentPage(MCPage* MP){currentPage=MP;}
        MachineFunction *getCurrentMF(){ return currentMF;}
        MCFragment* getFuncFragmentStart(){ return funcFragmentStart;}
        MCPage* getCurrentPage(){ return currentPage;}

        unsigned getStartSecOffset(){ return instrStartSecOffset;}
        void setEndSecOffset(int funcSize){
            prevFuncInstrEndSecOffset=instrStartSecOffset + funcSize;
        }
    };

    CurFunctionStatic CFStatic;
    CurFunctionStatic & getCFStatic(){ return CFStatic;}
    virtual void setCurrentMFStatic(MCSection * MCS,MachineFunction *MF){CFStatic.setCurrentMF(MF);}
    virtual void setStartFragmentStatic(){}
    virtual void setCurrentPageStatic(){}
    virtual bool addFuncToPage(MCSection* MCS,MCSymbol *MF, int approxSize){ return false;}
//    virtual void dumpPageInfo(){
//        MCSection::page_iterator PB=getCurrentSectionOnly()->getPageList().begin(),
//                PE=getCurrentSectionOnly()->getPageList().end();
//        for(; PB!=PE ; ++PB){
//          for(MCPage::iterator FB=PB->begin(),FE=PB->end();FB!=FE;FB++){
//
//          }
//        }
//    }
    virtual void EmitPageAlign(MCSection::iterator insertionPoint, unsigned pageWide){}
  ...
  virtual void EmitConstantFragment(int CPI, int size, unsigned align,MCSymbol *symbol= nullptr,MCSymbol *NSymbol= nullptr);
  ...
  virtual void EmitCodeAlignment(unsigned ByteAlignment,
                                 unsigned MaxBytesToEmit = 0);
  virtual void EmitMCBBstart(MCSymbol *MBB,bool);
  virtual void EmitMCBBend(MCSymbol *MBB,MCSymbol * targetMBB = nullptr, MCSymbol *implicate= nullptr);
  ...
  virtual void EmitInstructionWithReplaceable(const MCInst *InstSrc, const MCInst * InstReplace_MOV, const MCInst * InstReplace_jp, const MCSymbol * targetBB,const MCSubtargetInfo &STI,
                               bool PrintSchedInfo = false);
  virtual void EmitInstructionInPageJmp(const MCInst *InstSrc, const MCSubtargetInfo &STI,
                                          bool PrintSchedInfo = false);
