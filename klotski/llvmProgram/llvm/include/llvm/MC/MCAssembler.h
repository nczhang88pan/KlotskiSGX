...
private:
  ...
  private:
      int index;
      int pageAlign;
      std::set<MCSymbol *> SymbolSet;
      MCSymbol* startFragment;
      MCDataFragment *constantFragment;

  public:
      MCAsmPage(int pageIndex):index(pageIndex),pageAlign(11){
          SymbolSet.clear();
      }
      ~MCAsmPage(){
          SymbolSet.clear();
      }
      void setConstantFragment(MCDataFragment *fragment){
          constantFragment=fragment;
      }
      MCDataFragment *getConstantFragment(){
          return constantFragment;
      }
      void addSymbolToPage(MCSymbol * symbol){
          SymbolSet.insert(symbol);
      }
      void removeSymbolFromPage(MCSymbol *symbol){
          SymbolSet.erase(symbol);
      }

      bool consultSymbolInPage(MCSymbol *symbol){
          auto search = SymbolSet.find(symbol);
          if(search!=SymbolSet.end())
              return  true;
          return false;
      }
      int getPageAlign(){
  ...
  std::list<MCAsmPage *> MCAsmPageList;
  std::map<MCSymbol*,MCAsmPage *> MCAsmPageMap;
  MCAsmPage*  currentMCAsmPage= nullptr;
  std::map<const MCSymbol*, MCMBBConstantFragment*> currentConstentMap;
  MCSymbol * currentJMP= nullptr;
    std::map<const MCSymbol *,const MCSymbol *> orgToNewSymbolMap;
  ...
  bool fixupNeedsRelaxation(const MCFixup &Fixup, const MCReplaceableFragment *DF,
                            const MCAsmLayout &Layout) const;

  /// Check whether the given fragment needs relaxation.
  bool fragmentNeedsRelaxation(const MCRelaxableFragment *IF,
                               const MCAsmLayout &Layout) const;
    ///Check whether the given fragment needs adjust.
  bool fragmentNeedsRelaxation(const MCReplaceableFragment *IF,
                                const MCAsmLayout &Layout) const;
  ...
  bool layoutSectionOnce(MCAsmLayout &Layout, MCSection &Sec);
    ...
    ///PZ: Related function about handling fragments in a page
  bool perCalculateAPage(MCAsmLayout &Layout,MCSection &Section, MCSection::iterator &start,MCSection::iterator &SectionEnd);
  bool calculateApage(MCAsmLayout &Layout,MCSection &Section, MCSection::iterator start,MCSection::iterator &SectionEnd);
  int getFragmentSize(MCFragment *itr,int PageSize,int BlockSize,int PageConstantSize,int *BlockGenreatedConstantSize,std::map<const MCSymbol*, MCMBBConstantFragment*> & tempMAP);
  void insertConstantFragment(MCAsmLayout &Layout,MCSection &Section);


  bool relaxInstruction(MCAsmLayout &Layout, MCRelaxableFragment &IF);

  bool relaxReplaceInstruction(MCAsmLayout &Layout,MCReplaceableFragment &IF);
...
public:
  ...
  void alignDataSection(MCAsmLayout &Layout);
