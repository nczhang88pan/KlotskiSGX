...
MCSymbol *MachineBasicBlock::getSymbol() const {
  if (!CachedMCSymbol) {
    ...
    CachedMCSymbol = Ctx.getOrCreateSymbol(Twine(Prefix) + "_BB" +
...
MCSymbol* MachineBasicBlock::buildSymbolForMBBStart() const {
    if(!CachedMCSymbolStart){
        const MachineFunction *MF = getParent();
        MCContext &Ctx = MF->getContext();
//        auto moduleName = MF->getFunction()->getParent()->getName();

        CachedMCSymbolStart = Ctx.getOrCreateSymbol( "MBBStart_" +
                                               Twine(MF->getName()) +
                                               "_" + Twine(getNumber()));
    }
    return CachedMCSymbolStart;
}
