...
unsigned lableI=0;
MCSymbol* MachineFunction::getCallNextInstrLableSymbole() const {
    const DataLayout &DL = getDataLayout();
    return Ctx.getOrCreateSymbol(Twine(DL.getPrivateGlobalPrefix()) +Twine(lableI++)+
                                 Twine(getFunctionNumber()) + "$pb");

}
MCSymbol* MachineFunction::getCallNextInstrLableSymbole(const Twine &Name) const {
    const DataLayout &DL = getDataLayout();
    return Ctx.getOrCreateSymbol(Name+"_"+Twine(lableI++));
}
