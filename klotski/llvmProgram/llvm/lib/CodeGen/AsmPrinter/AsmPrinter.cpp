...
void AsmPrinter::EmitMBBend(const MachineBasicBlock &MBBC) {
    MachineBasicBlock *MBB = const_cast<MachineBasicBlock*>(&MBBC);
    ...
    if(ME!=MBB->end()){
        switch (ME->getOpcode()) {
            ...
            case TargetOpcode::KILL:
                errs()<<__func__<<"not an instruction "<<"\n";
                ME->print(errs());errs()<<"\n";
                exit(-1);
                break;
            default:
    ...
    EmitMBBEndMIToMCBBEndFragment(MBBC,ME,MBB->end());
    return;
}
void AsmPrinter::SplitLargeBlock(MachineBasicBlock &MBB) {
    if(MBB.size()>400){
        unsigned instructionNum=0;
        MachineFunction *MF = MBB.getParent();
        const BasicBlock *LLVM_BB=MBB.getBasicBlock();
        MachineBasicBlock *SecondMBB = MF->CreateMachineBasicBlock(LLVM_BB);
        MachineFunction::iterator MBBIter = std::next(MBB.getIterator());
        MF->insert(MBBIter,SecondMBB);

        MachineBasicBlock::iterator MBI=MBB.begin();
        for(;instructionNum<MBB.size()/2;MBI++,instructionNum++){}

        //Split MBB and move the tail portion down to ContinueMBB

        SecondMBB->splice(SecondMBB->begin(),&MBB,MBI,MBB.end());
        SecondMBB->transferSuccessorsAndUpdatePHIs(&MBB);
        MBB.addSuccessor(SecondMBB);
        SplitLargeBlock(MBB);
        SplitLargeBlock(*SecondMBB);
    }
    return;
}
...
void AsmPrinter::EmitFunctionBody() {
    MachineFunction *machineFunction = const_cast<MachineFunction *>(MF);
    for(auto &MBB: *machineFunction){
        if(MBB.size()>400){
            errs()<<MBB.buildSymbolForMBBStart()->getName()<<" "<<MBB.size()<<" current MBB over 400\n";
            SplitLargeBlock(MBB);
        }
    }
  ...
  for (auto &MBB : *MF) {
    ...
    auto MB = MBB.begin();
    auto ME = MBB.getFirstTerminator();

      //PZ: in this loop, the body only handles the instructions that rang from the begin to the first terminator
    for (;MB!=ME;MB++) {
      auto &MI = *MB;

      EmitMIConstantRecordFragment(MI);
      ...
      //PZ:: handle the Terminator instruction
    EmitMBBend(MBB);


    EmitBasicBlockEnd(MBB);
  }
//    errs()<<"+++the end of a function\n";
...
bool sortBySize(const GlobalVariable &L, const GlobalVariable &R) {

    const DataLayout &DL = L.getParent()->getDataLayout();

    uint64_t Size_a = DL.getTypeAllocSize(L.getType()->getElementType());
    uint64_t Size_b = DL.getTypeAllocSize(R.getType()->getElementType());
  ...
  for (unsigned i = 0, e = CPSections.size(); i != e; ++i) {
    for (unsigned j = 0, ee = CPSections[i].CPEs.size(); j != ee; ++j) {
      ...
      //get generated code size
        MCDataFragment * fragment = dyn_cast_or_null<MCDataFragment>(Sym->getFragment());
        if(fragment){

            int size = getDataLayout().getTypeAllocSize(Ty);

            MBBConstantMap.insert(std::make_pair(Sym,size));

            MBBConstantAlignMap.insert(std::make_pair(Sym,CPE.getAlignment()));
...
void AsmPrinter::EmitBasicBlockStart(const MachineBasicBlock &MBB) const {
  ...
  MCSymbol *MBBsymbol = MBB.buildSymbolForMBBStart();
    //get function symbol
    MCSymbol*symbol=getSymbol(MBB.getParent()->getFunction());
    MCFragment *fragment=symbol->getFragment();
    if(!fragment){
        errs()<<MBB.getParent()->getFunction()<<" doest not have fragment\n";
    }


  if(MBBsymbol==MBB.getParent()->begin()->buildSymbolForMBBStart()){
      OutStreamer->EmitMCBBstart(MBBsymbol,true);
  } else{
      OutStreamer->EmitMCBBstart(MBBsymbol,false);
  }
