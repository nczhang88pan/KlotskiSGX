...
void MCELFStreamer::EmitInstToRepalce(const MCInst &Inst, const MCSubtargetInfo &STI, const MCReplaceableFragment &fragment,SmallString<256> &Code) {
    MCAssembler &Assembler = getAssembler();
    SmallVector<MCFixup, 4> Fixups;
    raw_svector_ostream VecOS(Code);
    Assembler.getEmitter().encodeInstruction(Inst, VecOS, Fixups, STI);

    MCReplaceableFragment * RF= const_cast<MCReplaceableFragment*>(&fragment);
    for (unsigned i = 0, e = Fixups.size(); i != e; ++i)
        fixSymbolsInTLSFixups(Fixups[i].getValue());

    // Add the fixups and data.
    for (unsigned i = 0, e = Fixups.size(); i != e; ++i) {
        Fixups[i].setOffset(Fixups[i].getOffset() + RF->getContents().size());
        RF->getFixups().push_back(Fixups[i]);
    }

    RF->getContents().append(Code.begin(), Code.end());
    return;
}
