...
void MCStreamer::EmitConstantFragment(int CPI, int size, unsigned Align, MCSymbol *symbol,MCSymbol *NSymbol) {}
...
void MCStreamer::EmitInstructionInPageJmp(const MCInst *Inst, const MCSubtargetInfo &STI,
                              bool ){
    // Scan for values.
    for (unsigned i = Inst->getNumOperands(); i--;)
        if (Inst->getOperand(i).isExpr())
...
void MCStreamer::EmitInstructionWithReplaceable(const MCInst * InstSrc, const MCInst * InstReplace_MOV, const MCInst * InstReplace_J,const MCSymbol * targetBB, const MCSubtargetInfo &STI,
                                            bool ){
    // Scan for values.
    if(InstSrc){
        for (unsigned i = InstSrc->getNumOperands(); i--;)
            if (InstSrc->getOperand(i).isExpr())
                visitUsedExpr(*InstSrc->getOperand(i).getExpr());
    }

    // Scan for InstReplace_MOV values.
    for (unsigned i = InstReplace_MOV->getNumOperands(); i--;)
        if (InstReplace_MOV->getOperand(i).isExpr())
            visitUsedExpr(*InstReplace_MOV->getOperand(i).getExpr());

    // Scan for InstReplace_J values.
    for (unsigned i = InstReplace_J->getNumOperands(); i--;)
        if (InstReplace_J->getOperand(i).isExpr())
            visitUsedExpr(*InstReplace_J->getOperand(i).getExpr());
}
...
void MCStreamer::EmitCodeAlignment(unsigned ByteAlignment,
                                   unsigned MaxBytesToEmit) {}
void MCStreamer::EmitMCBBstart(MCSymbol *MBB,bool isFunctionBegin) {}
void MCStreamer::EmitMCBBend(MCSymbol *MBB, MCSymbol *targetMBB, MCSymbol *implicate) {}
