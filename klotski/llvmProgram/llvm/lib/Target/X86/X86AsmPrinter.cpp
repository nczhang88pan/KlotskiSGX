...
bool X86AsmPrinter::runOnMachineFunction(MachineFunction &MF) {
  if(MF.getName().startswith("addressDereference64")){
    return false;
  }
  if(MF.getName().startswith("Redereference64")){
    return false;
  }

  Subtarget = &MF.getSubtarget<X86Subtarget>();
//  MCSection *mcSection = OutStreamer->getContext().getObjectFileInfo()->getTextSection();

//    OutStreamer->setCurrentMFStatic(mcSection,&MF);
////   errs()<<MF.getName()<<" ::::"<<OutStreamer->getCFStatic().getStartSecOffset()<<"\n";
//   IC.reset(MF);
//   IC.setCodeSize(0);
//   IC.setInstrNum(0);
    //align the function,and remove nop instruction between MBBs
  ...
  EmitFunctionBody();
  //get the size of this function, however because of the relaxfragment, we can only use this as a estimate value.
//  OutStreamer->getCFStatic().setEndSecOffset(IC.getCodeSize()*1.1);
  //OutStreamer->getCFStatic().setEndSecOffset(generateCodeSize);
  //errs()<<MF.getName()<<"IC stastic instrution num: "<<IC.getInstrNum()<<"\n";
  //if is true, this function is added to a new page. so align it by a page
// if(OutStreamer->addFuncToPage(mcSection,
//                      CurrentFnSym,IC.getCodeSize()*1.1)){
////    if(OutStreamer->addFuncToPage(mcSection,
////                      CurrentFnSym,generateCodeSize)){
//
//
//       MF.setAlignment(1<<mcSection->getShadowPageWide());
// }
//  IC.free();
...
void X86AsmPrinter::InstCounter::addExtern(int i) {
    codeSize+=i;
}
void X86AsmPrinter::InstCounter::countInline(int i) {
    codeSize+=i;
    instNum++;
}
...
void X86AsmPrinter::EmitMIConstantRecordFragment(const MachineInstr &MI) {
    ...
    //get the CPI of this constant
    int cpi=MI.getOperand(AddrOffset+3).getIndex();
    MCSymbol *symbol = GetCPISymbol(cpi);
    auto itr = MBBConstantMap.find(symbol);
    if(itr != MBBConstantMap.end()){
        assert(!currentConstantNewSymbol && "currentConstantNewSymbol is not null");
        int size=itr->second;
        currentConstantNewSymbol= OutContext.getOrCreateSymbol(Twine(symbol->getName())+"_Index_"+Twine(constantIndex++));
        unsigned align=MBBConstantAlignMap.find(symbol)->second;
        OutStreamer->EmitConstantFragment(cpi,size,align,symbol,currentConstantNewSymbol);
//        errs()<<currentConstantNewSymbol->getName()<<"\n";
    }
//    EmitInstruction(&MI);
