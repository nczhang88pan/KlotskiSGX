...
#include "X86Rerandomization/testFunctions.h"
...
//  IC.count(Inst,getSubtargetInfo());//count the instruction and iteratly add its size
  OutStreamer->EmitInstruction(Inst, getSubtargetInfo(), EnablePrintSchedInfo);
  SMShadowTracker.count(Inst, getSubtargetInfo(), CodeEmitter.get());
}
void X86AsmPrinter::EmitAndGluePages(MCInst &Inst) {
    if(MF->hasInlineAsm()){
...
void X86AsmPrinter::EmitInlineAsmPan(const MachineInstr *MI) {
    int i = 0;
    if(OutStreamer->getCurrentSectionOnly()->end()!=OutStreamer->getCurrentSectionOnly()->begin()){
        MCDataFragment *F = dyn_cast_or_null<MCDataFragment>(&*std::prev(OutStreamer->getCurrentSectionOnly()->end()));
        if(F){
            i=F->getContents().size();
        } else{
//            errs()<<"I am here, not find start\n";
        }
    ...
    if(OutStreamer->getCurrentSectionOnly()->end()!=OutStreamer->getCurrentSectionOnly()->begin()) {
        MCDataFragment *F = dyn_cast_or_null<MCDataFragment>(&*std::prev(OutStreamer->getCurrentSectionOnly()->end()));
        if(F){
            i=F->getContents().size() - i;
        } else{
            errs()<<"I am here, not find end\n";
        }
    }
    IC.countInline(i);
}
void X86AsmPrinter::EmitInPageJmp(const MachineFunction *MF) {
    if(!inPageJumpSymbol){
        MCSymbol * trumk = MF->getContext().getOrCreateSymbol("MCASM_IN_PAGE_JMP");
        MCInst mcJmp = MCInstBuilder(X86::JMP64m).addReg(X86::R15)
                .addImm(1).addReg(0).addImm(0x90).addReg(0);

        OutStreamer->EmitInstructionInPageJmp(&mcJmp,getSubtargetInfo());
        inPageJumpSymbol =trumk;
    }
}
void X86AsmPrinter::EmitMBBEndMIToMCBBEndFragment(const MachineBasicBlock &MBBC, MachineBasicBlock::iterator &Start,MachineBasicBlock::iterator End) {
    MCInst mcISrc;
    ...
    for(;MBBI!=End;MBBI++){
        switch (MBBI->getOpcode()) {
            ...
            case X86::JMP_1:{
                //mov target_1,%r14
                //j_  tempTarget
                if(MBBI->getOperand(0).isMBB()){
                    targetMBB = MBBI->getOperand(0).getMBB()->buildSymbolForMBBStart();
                } else{
                    errs()<<"__func__"<<"Operand(0) is not MBB";
                    MBBI->print(errs());
                }
                MCInstLowering.Lower(&*MBBI, mcISrc);
//                errs()<<"Pair:::\n";
//                MBBI->print(errs());
//                mcISrc.print(errs());errs()<<"\n";
//                errs()<<"====Pair:::\n";
                MCInst mcInst = MCInstBuilder(mcISrc.getOpcode()).addExpr(MCSymbolRefExpr::create(targetMBB, OutContext));

                MCInst mcMOV = MCInstBuilder(X86::MOV64ri32).addReg(X86::R14).addExpr(MCSymbolRefExpr::create(targetMBB,OutContext));
                MCInst mcJMP = MCInstBuilder(mcISrc.getOpcode()).addExpr(MCSymbolRefExpr::create(inPageJumpSymbol, OutContext));
                OutStreamer->EmitInstructionWithReplaceable(&mcInst,&mcMOV,&mcJMP,targetMBB,getSubtargetInfo(), EnablePrintSchedInfo);
                break;
            }
//            case X86::JMP_1:
//                //mov target_1,%r14
//                //jmp tempTarget
//                //there is no fallthrouth here
...
//                return;
            ...
            case X86::RETQ:
                ...
                break;
            default:
                errs()<<__func__<<" find instruction that has not been handled\n";
                MBBI->print(errs());errs()<<"\n";
                exit(-1);
        }
    }

    //generate a fragment for the fallthrough
    MachineBasicBlock * fallthroughMBB=const_cast<MachineBasicBlock &>(MBBC).getFallThrough();
    if(fallthroughMBB){
        targetMBB = fallthroughMBB->buildSymbolForMBBStart();
        MCInst mcMOV = MCInstBuilder(X86::MOV64ri32).addReg(X86::R14).addExpr(MCSymbolRefExpr::create(fallthroughMBB->buildSymbolForMBBStart(),OutContext));
        MCInst mcJMP = MCInstBuilder(X86::JMP_1).addExpr(MCSymbolRefExpr::create(inPageJumpSymbol, OutContext));
        OutStreamer->EmitInstructionWithReplaceable(nullptr,&mcMOV,&mcJMP,targetMBB,getSubtargetInfo(), EnablePrintSchedInfo);
    }
    return;
}
void X86AsmPrinter::adjustInstructionContainCPI(const MachineInstr *MI, MCInst &TmpInst) {

    if(!MI->mayLoadOrStore() || isPOPorPush(MI->getOpcode()) || MI->isInlineAsm())
        return;
    ...
    const MachineOperand &baseR = MI->getOperand(AddrOffset);

    if(!(baseR.isReg() && baseR.getReg() == X86::RIP))
        return;

    if(!MI->getOperand(AddrOffset+3).isCPI())
        return;

    if(currentConstantNewSymbol){
        TmpInst.getOperand(AddrOffset+3).setExpr(MCSymbolRefExpr::create(currentConstantNewSymbol,OutContext));
        currentConstantNewSymbol= nullptr;
    }
}
...
#define CASE_ALL_MOV_RM()               \
  ...
  adjustInstructionContainCPI(MI,TmpInst);
  ...
  if(MI->isReturn()){
      ...
      if(isRET(opcode)){
          //pop  %%r14
          //jmp  0x40(%%r15)
          EmitAndCountInstruction(MCInstBuilder(X86::POP64r).addReg(X86::R14));
          EmitAndCountInstruction(MCInstBuilder(X86::TAILJMPm64)
                                            .addReg(X86::R15)
                                            .addImm(1)
                                            .addReg(0)
                                            .addImm(0x80)
...
#define finish 1
#if finish
      //TODO handle the call with JMP
      //first we need to check whether this target is in our optimize list;
      //Pan-pathOram: replace call with:
          //push $lable
          //mov  $targetFunction,%%r14
          //jmp  0x40(%%r15)
      //$lable_PIC:
      //[next instrs]
      if(MF->getName()=="fillPagetable" || MF->getName()=="main" || MF->getName()=="memcpy" || MF->getName()=="memset"){
//          IC.count(TmpInst,getSubtargetInfo());
          OutStreamer->EmitInstruction(TmpInst, getSubtargetInfo());
          return;
      } else{
          bool isTailJmp=false;
          if(opcode >=X86::TAILJMPd && opcode<=X86::TAILJMPr64_REX){
              isTailJmp = true;
          }

          //if it is normal call, we first push its next instruction's address to the stack.
          MCSymbol *PICBase;


          //then we should mov the address of the target function to the %%r14

//          if(isTailJmp){
//              //Pan-pathOram: in tailJmp we should do:
//              // mov  $targetFunction,%%r14
//              // jmp 0x40(%%r15)
//              IC.count(TmpInst,getSubtargetInfo());
//              OutStreamer->EmitInstruction(TmpInst, getSubtargetInfo());
//              return;
//          }
          if(opcode==X86::CALL64pcrel32 || opcode==X86::TAILJMPd64){
              if(MI->getOperand(0).isGlobal() && MI->getOperand(0).getGlobal()->getName().startswith("addressDereference64")){

                  EmitAndCountInstruction(MCInstBuilder(X86::CALL64m)
                                                  .addReg(X86::R15)
                                                  .addImm(0).addReg(0).addImm(0x60)
                                                  .addReg(0));
                  return;
              }

              EmitAndCountInstruction(MCInstBuilder(X86::MOV64ri32).addReg(X86::R14).addOperand(TmpInst.getOperand(0)));

          }else if(opcode==X86::CALL64m|| opcode==X86::TAILJMPm64){
//              errs()<<MI->getParent()->getParent()->getName()<<"\n";
              if(MI->getOperand(0).getReg()!=X86::R15){
                  EmitAndCountInstruction(MCInstBuilder(X86::MOV64rm)
                                                  .addReg(X86::R14)
                                                  .addOperand(TmpInst.getOperand(0))
                                                  .addOperand(TmpInst.getOperand(1))
                                                  .addOperand(TmpInst.getOperand(2))
                                                  .addOperand(TmpInst.getOperand(3))
                                                  .addOperand(TmpInst.getOperand(4)));
              }else if(MI->getOperand(3).getImm()==0x60){
                  EmitAndCountInstruction(MCInstBuilder(X86::CALL64m)
                                                  .addReg(X86::R15)
                                                  .addImm(0).addReg(0).addImm(0x60)
                                                  .addReg(0));
...
//              EmitAndCountInstruction(TmpInst);
          ...
          }else if(opcode==X86::CALL64r || opcode==X86::TAILJMPr64){
              EmitAndCountInstruction(MCInstBuilder(X86::MOV64rr).addReg(X86::R14).addOperand(TmpInst.getOperand(0)));
          }else{
                  errs()<<"path OPCODE:is "<<opcode<<"\n";
                  MI->print(errs());
                  errs()<<__func__<<" CALL not handled\n";
                  MI->getParent()->print(errs());
          }

          if(!isTailJmp){
              if(MI->getOperand(0).isGlobal()){
                  PICBase = MF->getCallNextInstrLableSymbole("Call_" + Twine(MI->getOperand(0).getGlobal()->getName()));
              } else{
                  PICBase = MF->getCallNextInstrLableSymbole();
              }

              EmitAndCountInstruction(MCInstBuilder(X86::PUSH64i32)
                                              .addExpr(MCSymbolRefExpr::create(PICBase,OutContext)));
          }

          //Then jmp to the dereference function 0x40(%r15)
