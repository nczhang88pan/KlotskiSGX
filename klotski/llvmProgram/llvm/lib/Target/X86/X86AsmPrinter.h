...
class LLVM_LIBRARY_VISIBILITY X86AsmPrinter : public AsmPrinter {
  ...
  MCSymbol *inPageJumpSymbol= nullptr;
  ...
  private:
    ...
    class InstCounter{
        public:
        InstCounter(TargetMachine &TM):TM(TM),CodeEmitter(NULL){}
        ~InstCounter(){};
        void count(MCInst &Inst, const MCSubtargetInfo &STI);
        void countInline(int i);
        void addExtern(int i);
        int get(){ return instSize;}
        int getCodeSize(){ return codeSize;}
        void setCodeSize(unsigned size){codeSize=size;}
        void setInstrNum(unsigned num){instNum=num;}
        unsigned getInstrNum(){ return instNum;}
       ...
       InstCounter IC;
       unsigned units;
       class MCObjectPageFameOptimize{
            public:
               MCObjectPageFameOptimize();
                ~MCObjectPageFameOptimize();
  ...
  void EmitAndGluePages(MCInst &Inst);
...
public:
  ...
  void EmitInstruction(const MachineInstr *MI) override;
    void adjustInstructionContainCPI(const MachineInstr *MI,MCInst &);
  void EmitMBBEndMIToMCBBEndFragment(const MachineBasicBlock &MBB, MachineBasicBlock::iterator &, MachineBasicBlock::iterator ) override;
  void EmitInPageJmp(const MachineFunction *MF)override ;
    void EmitInlineAsmPan(const MachineInstr *MI) override;
  ...
  void EmitMIConstantRecordFragment(const  MachineInstr &) override;
