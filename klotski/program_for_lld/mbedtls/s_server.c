...
extern void sgx_measure_start(double iter);
...
//#include <signal.h>
//#endif
char buff_server[BUFSIZ] = {'\0'};
int printf_sgx(const char *fmt, ...)
{
    int ret;
    va_list ap;
    va_start(ap, fmt);
    vsprintf(buff_server, fmt, ap);
    va_end(ap);
    puts(buff_server);
    return ret;
}
...
int version_suite(char *p,int version_suites[4][2],int *ret){
     int i;
...
int init_psk(char *p,psk_entry *psk_info,unsigned char psk[MBEDTLS_PSK_MAX_LEN],size_t* psk_len,const char *alpn_list[10]){
    int i;
    #if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
    /*
...
int waitingConnect_3(int *returnValue){
    int ret=*returnValue;;
       /*
...
int handshake_4(int *returnValue){
    int ret = *returnValue;
    #if PANSDEBUG
...
void verify_server_certificate_5(){
...
int read_HTTP_request_6(int *returnValue,int *lenValue){
    int ret = *returnValue;
    int len = *lenValue;
 /*
...
int write_200_Response_7(int *returnValue,int *lenValue,int *written,int *frags){
    int ret = *returnValue;
    int len = *lenValue;
     /*
...
int request_renegotiation_7a(int *returnValue,int *exchanges_left){
     int ret = *returnValue;
#if defined(MBEDTLS_SSL_RENEGOTIATION)
...
void server_exit(int *returnValue){
    int ret = *returnValue;
...
switch (init_opt(&ret)){
    case 1: goto usage;break;
    case 2: goto exit;break;
    default: break;
}

switch (version_suite(p,version_suites,&ret)){
    case 1: goto usage;break;
    case 2: goto exit;break;
    default: break;
}

switch (init_psk(p,psk_info,psk,&psk_len,alpn_list)){
    case 1: goto usage;break;
    case 2: goto exit;break;
    default: break;
}
...
* 6. Read the HTTP Request
*/
switch (read_HTTP_request_6(&ret,&len)){
    case 1: goto close_notify;break;
    case 2: goto reset;break;
    default: break;
}
    /*
...
if (request_renegotiation_7a(&ret,&exchanges_left)!=0){
    goto reset;
}
/*
* 7. Write the 200 Response
*/
if (write_200_Response_7(&ret,&len,&written,&frags)!=0){
   goto reset;
}
